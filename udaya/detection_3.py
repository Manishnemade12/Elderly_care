...
class VideoBuffer:
    def __init__(self, pre_fall_seconds=20, post_fall_seconds=20, fps=30):
        self.pre_fall_frames = int(pre_fall_seconds * fps)
        self.post_fall_frames = int(post_fall_seconds * fps)
        self.fps = fps
        
        # Circular buffer for continuous recording (pre-fall)
        self.pre_fall_buffer = deque(maxlen=self.pre_fall_frames)
        
        # Linear buffer for post-fall recording
        self.post_fall_buffer = []
        self.recording_post_fall = False
        self.post_fall_count = 0
        
    def add_frame(self, frame):
        """Add frame to appropriate buffer"""
        # Always add to pre-fall circular buffer
        self.pre_fall_buffer.append(frame.copy())
        
        # If recording post-fall, add to post-fall buffer
        if self.recording_post_fall:
            if self.post_fall_count < self.post_fall_frames:
                self.post_fall_buffer.append(frame.copy())
                self.post_fall_count += 1
            else:
                # Finished recording post-fall frames
                self.recording_post_fall = False
    
    def start_post_fall_recording(self):
        """Start recording post-fall frames"""
        if not self.recording_post_fall:
            self.post_fall_buffer = []
            self.post_fall_count = 0
            self.recording_post_fall = True
            print("📹 Started recording post-fall footage...")
    
    def is_recording_complete(self):
        """Check if post-fall recording is complete"""
        return not self.recording_post_fall and len(self.post_fall_buffer) > 0
    
    def save_complete_incident_video(self, filename):
        """Save combined pre-fall and post-fall video"""
        total_frames = len(self.pre_fall_buffer) + len(self.post_fall_buffer)
        if total_frames == 0:
            print("No frames to save")
            return False
        
        # Get frame dimensions from first available frame
        if len(self.pre_fall_buffer) > 0:
            h, w = self.pre_fall_buffer[0].shape[:2]
        elif len(self.post_fall_buffer) > 0:
            h, w = self.post_fall_buffer[0].shape[:2]
        else:
            return False
        ...
        try:
            ...
            def add_timestamp_overlay(frame, text, color=(0, 255, 255)):
            ...
            for i, frame in enumerate(self.pre_fall_buffer):
                seconds_before = (len(self.pre_fall_buffer) - i) / self.fps
                timestamp_text = f"PRE-FALL: -{seconds_before:.1f}s"
                overlay_frame = add_timestamp_overlay(frame, timestamp_text, (0, 255, 255))
                out.write(overlay_frame)
            
            # Write post-fall frames
            for i, frame in enumerate(self.post_fall_buffer):
                seconds_after = (i + 1) / self.fps
                timestamp_text = f"POST-FALL: +{seconds_after:.1f}s"
                overlay_frame = add_timestamp_overlay(frame, timestamp_text, (0, 0, 255))
                out.write(overlay_frame)
            
            pre_duration = len(self.pre_fall_buffer) / self.fps
            post_duration = len(self.post_fall_buffer) / self.fps
            total_duration = pre_duration + post_duration
            
            print(f"Complete incident video saved: {filename}")
            print(f"  📹 Pre-fall: {pre_duration:.1f}s ({len(self.pre_fall_buffer)} frames)")
            print(f"  🚨 Post-fall: {post_duration:.1f}s ({len(self.post_fall_buffer)} frames)")
            print(f"  ⏱️ Total: {total_duration:.1f}s ({total_frames} frames)")
            return True
            
        except Exception as e:
            print(f"Error saving incident video: {e}")
            return False
        finally:
            out.release()
    
    def save_current_buffer(self, filename):
        """Save only current pre-fall buffer (for manual saves)"""
        if len(self.pre_fall_buffer) == 0:
        ...
        try:
            for frame in self.pre_fall_buffer:
                out.write(frame)
            print(f"Video saved: {filename} ({len(self.pre_fall_buffer)} frames, {len(self.pre_fall_buffer)/self.fps:.1f}s)")
            return True
        except Exception as e:
    ...
    def reset_post_fall_recording(self):
        """Reset post-fall recording state"""
        self.post_fall_buffer = []
        self.post_fall_count = 0
        self.recording_post_fall = False
...
class FallDetector:
    def __init__(self, fps=30):
        ...
        # Video buffer for saving clips
        self.video_buffer = VideoBuffer(pre_fall_seconds=20, post_fall_seconds=20, fps=fps)
        
        # Fall incident tracking
        self.incident_video_saved = False
            ...
            if new_state in [PersonState.FALLING, PersonState.FALLEN]:
                ...
                if self.consecutive_fall_frames >= 3:  # Confirm over multiple frames
                    ...
                    if not self.alert_triggered:
                        self.trigger_alert()
                        # Start post-fall recording
                        self.video_buffer.start_post_fall_recording()
            else:
                self.consecutive_fall_frames = 0
                if self.current_state in [PersonState.FALLING, PersonState.FALLEN] and \
                   new_state in [PersonState.STANDING, PersonState.SITTING]:
                    ...
                    self.incident_video_saved = False
                    self.video_buffer.reset_post_fall_recording()
                self.current_state = new_state
        
        # Check if post-fall recording is complete and save incident video
        if self.alert_triggered and not self.incident_video_saved and self.video_buffer.is_recording_complete():
            self.save_complete_incident_video()
    ...
    def trigger_alert(self):
        ...
        print("📹 Pre-fall footage captured, now recording post-fall...")
    ...
    def save_complete_incident_video(self):
        """Save the complete incident video (pre + post fall)"""
        filename = f"fall_incident_{int(time.time())}.mp4"
        
        # Save video in background thread to avoid blocking
        def save_video():
            if self.video_buffer.save_complete_incident_video(filename):
                print(f"✅ Complete fall incident video saved: {filename}")
                print("📧 Ready to send to emergency contacts/medical team")
            else:
                print("❌ Failed to save complete incident video")
        
        video_thread = threading.Thread(target=save_video, daemon=True)
        video_thread.start()
        
        self.incident_video_saved = True
        
        # Here you could add:
        # - Send SMS/Email notification with video attachment
        # - Trigger IoT alarm
        # - Call emergency contacts
        # - Send to monitoring station
        # - Upload video to cloud storage
        # - Notify medical team with complete incident footage
...
def main():
    ...
    print("Enhanced Fall Detection System for Elderly Care - Started")
    print("🎥 Recording last 20 seconds continuously + 20 seconds post-fall")
    print("Press 'q' to quit, 's' to save current snapshot, 'v' to save current video buffer")
    print("-" * 70)
    ...
    while True:
        ...
        # Additional info
        pre_buffer_frames = len(detector.video_buffer.pre_fall_buffer)
        pre_buffer_seconds = pre_buffer_frames / fps
        
        post_status = ""
        if detector.video_buffer.recording_post_fall:
            post_frames = len(detector.video_buffer.post_fall_buffer)
            post_seconds = post_frames / fps
            post_status = f" | Post-fall: {post_seconds:.1f}s"
        
        info_text = f"FPS: {display_fps:.1f} | Pre-buffer: {pre_buffer_seconds:.1f}s{post_status}"
        cv2.putText(frame, info_text, (10, 100),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)
        
        # Recording indicator with enhanced status
        if detector.video_buffer.recording_post_fall:
            # Red recording indicator for post-fall
            cv2.circle(frame, (w - 30, 30), 10, (0, 0, 255), -1)
            cv2.putText(frame, "POST", (w - 65, 35),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        else:
            # Standard recording indicator
            cv2.circle(frame, (w - 30, 30), 8, (0, 100, 255), -1)
        ...
        if state in [PersonState.FALLING, PersonState.FALLEN]:
            ...
            if detector.video_buffer.recording_post_fall:
                status_text = "Recording post-fall footage..."
            elif detector.incident_video_saved:
                status_text = "Complete incident video saved!"
            else:
                status_text = "Capturing incident footage..."
                
            cv2.putText(alert_overlay, status_text, (w//4 + 20, h//2 + 40),
        ...
        # Display frame
        cv2.imshow("Enhanced Elderly Fall Detection System", frame)
        ...
        if key == ord('q'):
            ...
            # Manually save current pre-fall buffer
            filename = f"manual_video_{int(time.time())}.mp4"
            def save_manual_video():
                if detector.video_buffer.save_current_buffer(filename):
                    print(f"🎥 Manual video saved: {filename}")
                else:
                    print("❌ Failed to save manual video")
            
            video_thread = threading.Thread(target=save_manual_video, daemon=True)
            video_thread.start()
